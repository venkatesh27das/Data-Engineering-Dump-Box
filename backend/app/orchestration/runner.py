...
from app.domain.assets import (
    Concept, Entity, Event, Fact, GraphSchema, KnowledgeAssetPackage, QualityDisposition,
...
class RunOrchestrator:
    ...
    def _build_graph(self, run_id: str, project: Project, sources: list[SourceAsset]) -> StateGraph:
        ...
        async def source_analysis(state: OrchestrationState) -> OrchestrationState:
            ...
            return {"normalized_sources": [item.model_dump(mode="json") for item in normalized], "replan_reason": reason}
        ...
        async def knowledge_engineering(state: OrchestrationState) -> OrchestrationState:
            ...
            concepts = await discover_concepts(self.provider, normalized, objective)
            entities = await extract_entities(self.provider, normalized, objective)
            ...
            if project.graph_depth != GraphDepth.METADATA:
                relationships = await extract_relationships(self.provider, normalized, objective, resolution.entities)
            if project.graph_depth in {GraphDepth.SEMANTIC, GraphDepth.CONTEXTUAL}:
                semantic_mappings = await map_semantics(self.provider, normalized, objective, concepts)
            if project.graph_depth == GraphDepth.CONTEXTUAL:
                facts = await extract_facts(self.provider, normalized, objective)
                events = await extract_events(self.provider, normalized, objective)
        ...
        async def graph_modelling(state: OrchestrationState) -> OrchestrationState:
            ...
            return {"graph_schema": schema.model_dump(mode="json")}
        ...
        async def quality_review(state: OrchestrationState) -> OrchestrationState:
            ...
            return {"quality_report": report.model_dump(mode="json"), "package": package.model_dump(mode="json")}
        ...
        builder.add_conditional_edges("quality_reviewer", quality_route, {"replan": "replan", "complete": "finalize"})
